...
export class WorkflowRelationshipHelper {
  ...
  async updateActionRelationships(
    workflowId: number,
    actionRelationships: Map<
      number,
      { nextActionId?: number; parentActionId?: number }
    >,
  ): Promise<void> {
    this.logger.log(
      `Updating ${actionRelationships.size} action relationships for workflow ${workflowId}`,
    );
    const updates = [];
    ...
    for (const [actionId, relationships] of actionRelationships.entries()) {
      ...
      if (relationships.nextActionId !== undefined) {
        ...
        this.logger.debug(
          `Action ${actionId}: nextActionId=${relationships.nextActionId}`,
        );
      }
      if (relationships.parentActionId !== undefined) {
        updateData.parentActionId = relationships.parentActionId;
        this.logger.debug(
          `Action ${actionId}: parentActionId=${relationships.parentActionId}`,
        );
      }
      ...
      if (Object.keys(updateData).length > 0) {
        ...
        this.logger.debug(
          `Queued update for action ${actionId}: ${JSON.stringify(updateData)}`,
        );
      }
    }

    this.logger.debug(
      `Executing ${updates.length} relationship updates in parallel`,
    );
    await Promise.all(updates);
    this.logger.log(
      `Successfully updated ${updates.length} action relationships for workflow ${workflowId}`,
    );
  }
